...
pub trait StreamExt: Sized {
    fn from_raw_fd(fd: RawFd, event_queue: &EventQueue) -> io::Result<Self>;
}

impl StreamExt for crate::Stream {
    fn from_raw_fd(fd: RawFd, event_queue: &EventQueue) -> io::Result<Self> {
        let registration = event_queue.register_fd(fd)?;
...
impl PreStream {
    pub fn register(self, event_queue: &EventQueue) -> io::Result<Stream> {
        let registration = event_queue.register_fd(self.fd)?;
