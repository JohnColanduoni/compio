...
fn bench_channel_echo(c: &mut Criterion) {
    ...
    executor.spawn(async move {
        ...
        let result: io::Result<()> = try {
            ...
            loop {
                let msg_len = await!(channel.read(&mut buffer))?;
                await!(channel.write(& buffer[..msg_len]))?;
            }
        };
    ...
    c.bench_function("channel_echo", move |bencher| bencher.iter(|| {
        executor.block_on(async {
            ...
            await!(channel.write(&buffer)).unwrap();
            await!(channel.read(&mut buffer)).unwrap();
        });
    }));
}
...
fn bench_channel_echo_cross_thread(c: &mut Criterion) {
    ...
    let background = thread::spawn(move || {
        let mut executor = LocalExecutor::new().unwrap();
        let mut channel = a.register(&executor.registrar().unwrap()).unwrap();
    ...
    let mut executor = LocalExecutor::new().unwrap();
    let mut channel = b.register(&executor.registrar().unwrap()).unwrap();   
